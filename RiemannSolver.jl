...
f(p, status::HydroStatus, ::Type{Shock}) = (p - status.p) * sqrt(status.A / (p + status.B))
f(p, status::HydroStatus, ::Type{Rarefaction}) = 2. * status.c / (status.gamma - 1.) * ((p / status.p)^((status.gamma - 1.) / 2. /status.gamma) - 1.)
...
function p_star_calc(left::HydroStatus, right::HydroStatus, guess_scheme::Type{T}, TOL::Float64) where {T<:FirstGuessScheme}	
...
rho_star_calc(p_star, status::HydroStatus, ::Type{Rarefaction}) = status.rho * (p_star / status.p) ^ (1. / status.gamma)

#equation (4.25) and (4.34) from Toro
c_star_calc(p_star, status::HydroStatus, ::Type{Rarefaction}) = status.c * (p_star / status.p) ^ ((status.gamma - 1.) / 2. / status.gamma)

#equations (4.50) and (4.57) from Toro
function rho_star_calc(p_star, status::HydroStatus, ::Type{Shock})
...
head_speed_calc(p_star, u_star, status::HydroStatus,
				side::Type{T}, ::Type{Rarefaction}) where {T<:Side} = status.u + side_factor(side) * status.c

#equation (4.55) and (4.62) from Toro 
function tail_speed_calc(p_star, u_star, status::HydroStatus, side::Type{T}, ::Type{Rarefaction}) where {T<:Side}
	c_star = c_star_calc(p_star, status, Rarefaction)
	return u_star + side_factor(side) * c_star
end

#equations (4.52) and (4.59) from Toro
function head_speed_calc(p_star, u_star, status::HydroStatus, side::Type{T}, ::Type{Shock}) where {T<:Side}
	gamma = status.gamma
...
tail_speed_calc(p_star, u_star, status::HydroStatus, side::Type{T}, ::Type{Shock}) where {T<:Side} = head_speed_calc(p_star, u_star, status, side, Shock)


#for vaccum generation the density in the middle is zero (= vaccum)
rho_star_calc(p_star, status::HydroStatus, ::Type{Vaccum}) = 0.
...
head_speed_calc(p_star, u_star, status::HydroStatus,
				side::Type{T}, ::Type{Vaccum}) where {T<:Side} = status.u + side_factor(side) * status.c
tail_speed_calc(p_star, u_star, status::HydroStatus, side::Type{T}, ::Type{Vaccum}) where {T<:Side} = status.u - side_factor(side) * 2 * status.c / (status.gamma - 1.)

#this is the density, velocity and pressure profile of a rarefaction wave in some specific x/t	
#see (4.56) and (4.63) from Toro
function rarefaction_profile(x_over_t, status::HydroStatus, side::Type{T}) where {T<:Side}
	gamma = status.gamma
...
function sample_riemann_regular(x, t::Float64, left::HydroStatus, right::HydroStatus,
								  guess_scheme::Type{T}, TOL::Float64) where {T<:FirstGuessScheme}
	@assert left.rho != 0.
	@assert right.rho != 0.
	...
	for i = 1:length(x)
		...
		right_condition = side == RightSide
...
function sample_riemann_side_vaccum(x, t::Float64, status::HydroStatus, side::Type{T}) where {T<:Side}
...
function sample_riemann(x, t::Float64, left::HydroStatus =  HydroStatus(1.,    0., 1.,  7. / 5.),
									   ...
									   guess_scheme::Type{T} = PrimitiveValue, TOL::Float64 = 1.e-6) where {T<:FirstGuessScheme}
